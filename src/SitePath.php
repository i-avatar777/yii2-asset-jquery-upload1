...
class SitePath
{
    ...
    /**
     * Проверяет отсутствует ли в файловой системе такой файл
     *
     * @return bool
     */
    ...
    {
        return is_dir($this->getPathFull());
    }
     ...
     * ]
     */
    public function getStructure($options = [], $files = [])
    {
        // получаю
        $arr = scandir($this->getPathFull());
        foreach ($arr as $item) {
            if (!in_array($item, ['.', '..'])) {
                $o = $this->cloneObject()->add($item);
                if ($o->isDirectory()) {
                    $d = $o->getStructure();
                    $files = array_merge($files, $d);
                } else {
                    $fullPath = $o->getPathFull();
                    $files[] = [
                        'path'  => $o->getPath(),
